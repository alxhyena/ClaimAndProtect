...
namespace xeonch\ClaimAndProtect\libs\multieconomy\providers;
...
class BedrockEconomyProvider extends EconomyProvider
{
    ...
    public function giveMoney(Player $player, float $amount, ?callable $callback = null): void
    {
        ...
        $this->api->add($player->getXuid(), $player->getName(), (int)$amount, $decimals,
    ...
    {
        $decimals = (int)(explode('.', strval($amount))[1] ?? 0);
        $this->api->subtract($player->getXuid(), $player->getName(), (int)$amount, $decimals,
            fn() => $callback ? $callback(true) : null,
            fn() => $callback ? $callback(false) : null
        );
    }
    ...
    {
        $decimals = (int)(explode('.', strval($amount))[1] ?? 0);
        $this->api->set($player->getXuid(), $player->getName(), (int)$amount, $decimals,
            fn() => $callback ? $callback(true) : null,
            fn() => $callback ? $callback(false) : null
        );
    }
