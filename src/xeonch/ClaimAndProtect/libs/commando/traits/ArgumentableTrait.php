...
namespace xeonch\ClaimAndProtect\libs\commando\traits;

use CortexPE\Commando\exception\ArgumentOrderException as ExceptionArgumentOrderException;
use pocketmine\command\CommandSender;
use xeonch\ClaimAndProtect\libs\commando\args\BaseArgument;
use xeonch\ClaimAndProtect\libs\commando\args\TextArgument;
use xeonch\ClaimAndProtect\libs\commando\BaseCommand;
use xeonch\ClaimAndProtect\libs\commando\exception\ArgumentOrderException;
	...
	public function registerArgument(int $position, BaseArgument $argument): void {
		...
		foreach($this->argumentList[$position - 1] ?? [] as $arg) {
			...
			if($arg->isOptional() && !$argument->isOptional()){
				throw new ExceptionArgumentOrderException("You cannot register a required argument after an optional argument");
			}
		}
