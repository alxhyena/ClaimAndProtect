...
class RemoveLandSubCommand extends BaseSubCommand
{
    protected function prepare(): void
    {
        $this->setPermission("claimandprotect.command.remove");
    ...
    public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
    {
        if ($sender instanceof Player) {
            if (isset($args['owner'])) {
                if ($sender->hasPermission("claimandprotect.admin.remove")) {
                    (new RemoveLandForm())->open($sender, $args['owner']);
                ...
                (new RemoveLandForm())->open($sender, $sender->getName());
            }
        }
    }
}
